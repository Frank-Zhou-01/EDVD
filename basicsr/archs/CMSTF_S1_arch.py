...
class CMCA(nn.Module):
    """
    Cross-Modal Collaborative Attention (CMCA)
    """
...
class FFN(nn.Module):
    """
    Dynamic Space Enhancement Module (DSEM)
    """
    def __init__(self, channels: int, expansion: float) -> None:
        ...
        self.e_conv1 = nn.Conv2d(channels, mid_channels, kernel_size=1)
        ...
        self.lin = nn.Linear(256, mid_channels)

    def forward(self, x: Tensor, e: Tensor, s: Tensor) -> Tensor:
        B, T, C, H, W = x.shape

        s = rearrange(s, 'N C H W -> N H W C')
        s = self.lin(s)
        s = rearrange(s, 'N H W C -> N C H W')

        x = rearrange(x, 'B T C H W -> B C T H W')
        x = self.conv_in(x)
        x1, x2 = torch.chunk(x, 2, dim=1)
        x1 = rearrange(x1, 'B C T H W -> (B T) C H W')
        x1 = self.dynamic_conv(x1, s)
        x1 = rearrange(x1, '(B T) C H W -> B C T H W', B=B)
        x2 = rearrange(x2, 'B T C H W -> (B T) C H W')
        x2 = x2 * rearrange(self.e_conv1(rearrange(e, 'B T C H W -> (B T) C H W')), '(B T) C H W -> B C T H W', B=B)
        x2 = rearrange(self.e_conv(rearrange(x2, 'B C T H W -> (B T) C H W')), '(B T) C H W -> B C T H W', B=B)
        x = x1 * x2
        x = self.conv_out(x)
        x = rearrange(x, 'B C T H W -> B T C H W')
        return x
...
class CMFFM(nn.Module):
    """
    Event-Guided Dynamic Feature Fusion Module (EGDFFM)
    """
    ...
    def forward(self, x1: Tensor, x2: Tensor, e: Tensor) -> Tensor:
        x1 = self.conv_e(torch.cat((x1, e), dim=1))
        ...
        x = x1 + x2
        x = self.dconv(x)
        x = self.conv4(torch.cat((x, e), dim=1))
        x = self.res_layer(x)
...
class ForwardFeatureFusion(nn.Module):
    ...
    def forward(self, x: Tensor, e: Tensor) -> Tensor:
        ...
        for t in range(0, T):
            ...
            fusion_feat = self.forward_fusion(x_feat, feat, e_feat)
...
class BackwardFeatureFusion(nn.Module):
    ...
    def forward(self, x: Tensor, e: Tensor) -> Tensor:
        ...
        for t in range(T - 1, -1, -1):
            ...
            fusion_feat = self.backward_fusion(x_feat, feat, e_feat)
...
class Deblur_S1_arch(nn.Module):
    ...
    def forward(self, x: Tensor, e: Tensor, gt: Tensor, use_LE: bool = True) -> Tensor:
        res0 = x
        B, T, C, H, W = x.shape
        ...
        e1 = rearrange(e1, '(B T) C H W -> B T C H W', B=B)
        x = self.Backward(x, e1)
        x = self.Forward(x, e1)
